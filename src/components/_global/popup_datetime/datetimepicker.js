...
function showMask(NUM) {
  ...
  if (!MASK[NUM]) {
    MASK[NUM] = toElement(MASK_TEMPLATE)
    document.body.appendChild(MASK[NUM])

    MASK[NUM].addEventListener('click', function () {
      CURRENT_PICKER[NUM] && CURRENT_PICKER[NUM].hide('cancel')
    }, false)
    MASK[NUM].addEventListener('touchmove', function (e) {
      e.preventDefault()
    }, false)
  }

  MASK[NUM].style.display = 'block'
  setTimeout(function () {
    if (MASK[0].style.opacity === '' || MASK[0].style.opacity === "0") {
      MASK[NUM] && (MASK[NUM].style.opacity = 0.5)
    }
  }, 0)
  MASK_NUM++
}
...
function hideMask(NUM) {
  ...
  MASK[NUM].style.opacity = 0
  //setTimeout(function () {
    MASK[NUM] && (MASK[NUM].style.display = 'none')
  //}, 200)
}
