...
export default {
    ...
    methods: {
        ...
        onChangeSearch(option) {
            ...
            setTimeout(() => {
                if (this.topArr) {
                    let index = this.topArr.findIndex(
                        (item) => item >= scrollTop || item == 0
                    );
                    index = index === -1 ? this.topArr.length : index;
            ...
            this.$nextTick(async () => {
                if (dataIndex !== 0) {
                    ...
                    for (let index in show) {
                        if (show[index] === true) {
                            const offsetTop =
                                this.$refs["data" + index][0].offsetTop - 95;
                            top.push(offsetTop);
                        } else {
