<!--健康接力-->
<template>
    <div class="share-money bg-grey" @touchstart.stop="touchstart">
        <div class="share-nav">
            ...
            <div class="item" v-if="!!experience.orgPhone" @click="callPhone(experience.orgPhone)">
                        ...
                        <div class="share-inner white">
                            <cy-btn @click.native="onShowShare" background="#29D7A8" angle lg>
                                <cy-row>
                                    ...
                                    <cy-col v-if="project.shareBackPrice>0" :span="18" class="tl lh12">
        ...
        <cy-popup
            ...
            background="#ffffff"
        >
            <div class="pdlr-40 pdt-40 pdb-25 tc">
                <cy-image :src="experience.qrCodeUrl" :lazy="false"></cy-image>
                <div class="grey">关 注 公 众 号</div>
            </div>
        </cy-popup>-->
...
export default {
    ...
    data() {
        return {
            ...
            //监听长时间未操作
            longCurrentTime: null, //当前时间
            longTimeOut: 2 * 60 * 1000, // 设置超时时间：2分钟
            longTimer: null, //超时定时器
            readyTimer: null, //所有数据加载完定时器
        };
    },
    created() {
        this.longCurrentTime = new Date().getTime(); // 记录当前时间
    ...
    mounted() {
        this.setIntervalTime(); // longTimeOut轮询一次，监听长时间未操作
    },
    computed: {
    ...
    methods: {
                ...
                .then(async (res) => {
                    ...
                    if (!this.orgPk) {
                        ...
                        if (
                            ...
                            this.getProjectOtherData(); //获取免费体验信息
                    ...
                    //设置分享
                    this.orgPk && this.browsePk && this.setMenuShare();
                    ...
                    //海报配置
                    this.orgPk && this.browsePk && this.posterQrCode();

                    this.$nextTick(() => {
                        if (res.obj.imgVideoList.length > 0) {
                            //监听轮播中的视频
                            this.addEventListenerSwiperVideo();
                        }
                        this.$refs.shareTab.bindStickyFresh(); //绑定
                    });
                })
                .catch((err) => {});
        },
        ...
        //     //查看体验
        //     if (
        //         this.experience.isExperience === "2" ||
        //         this.experience.isExperience === "3"
        //     ) {
        //         // 体验状态 门店简码 门店orgpk
        //         this.$router.push(
        //             `/experience_center/${this.experience.isExperience},${this.experience.experienceSimpleCode},${this.orgPk},${this.experience.experienceOrgName}`
        //         );
        //         //申请体验
        //     } else if (this.experience.isExperience === "1") {
        //         this.$router.push(
        //             `/experience?orgPk=${this.orgPk}&orgName=${this.experience.experienceOrgName}`
        //         );
        //     }
        // },
        ...
        updateBuyOrForwardStatus(type, callback) {
            ...
            this.$api
                .updateBuyOrForwardStatus(params)
                .then((res) => {
                    callback && callback();
                })
                .catch((err) => {
        ...
        //门店总数
        onChangeStore(totle) { console.log('totletotle',totle);
            if (totle === "") {
                this.$toast("附近无可用门店");
            }
        ...
        touchstart() {
            ...
            if (this.longTimer) {
                window.clearInterval(this.longTimer);
                this.longTimer = null;
            } else {
                this.setBrowseTime();
            }
            this.longCurrentTime = now; //更新当前时间
            this.setIntervalTime();
        },

        //轮询是否长时间未操作
        setIntervalTime() {
            this.longTimer = window.setInterval(() => {
                const now = new Date().getTime();
                //超时
                if (now - this.longCurrentTime > this.longTimeOut) {
                    if (this.browseTimer) {
...
.scroll-detail,.scroll-case,.scroll-evaluate{
    position: absolute;
    height: 1px;
    top: -40px;
}
